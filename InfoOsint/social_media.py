...
def search(username):
    ...
    for url in WEBSITES:
        try:
            ...
            if r.status_code == 200:
                if match == True:
                    print(GREEN + '[+] FOUND MATCHES' + RESET)
                    match = False
                print(YELLOW + f'\n{url} - {r.status_code} - OK' + RESET)
                if username in r.text:
                    print(GREEN + 'POSITIVE MATCH: Username:{} - text has been detected in url.'.format(username) + RESET)
                else:
                    print(GREEN +
                        f'POSITIVE MATCH: Username:{username} - \033[91mtext has NOT been detected in url, could be a FALSE POSITIVE.' + RESET)  #
            count += 1
    ...
    total = len(WEBSITES)
    print(GREEN + 'FINISHED: A total of {} MATCHES found out of {} websites.'.format(count, total) + RESET)
